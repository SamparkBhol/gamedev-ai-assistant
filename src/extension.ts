...
const weaponBalancing = {
    calculateDPS: (damage: number, speed: number, critRate: number, critMultiplier: number) => {
        const baseDPS = damage * speed;
        const critDPS = baseDPS * (1 + (critRate / 100) * (critMultiplier - 1));
        return Math.round(critDPS * 100) / 100;
    },
    
    suggestBalance: (weaponType: string, currentStats: any) => {
        const templates = {
        ...
        const template = templates[weaponType as keyof typeof templates];
        if (!template) return "Unknown weapon type";
...
const levelGenerator = {
    generateLayout: (size: string, theme: string) => {
        const layouts = {
        ...
        return `${layouts[size as keyof typeof layouts]}\n\nTheme Elements: ${themes[theme as keyof typeof themes]}`;
    }
};

const characterGenerator = {
    createConcept: (role: string, setting: string) => {
        const roles = {
        ...
        const roleTypes = roles[role as keyof typeof roles] || ["Generic Hero"];
        const randomType = roleTypes[Math.floor(Math.random() * roleTypes.length)];
        
        return {
            archetype: randomType,
            setting: settings[setting as keyof typeof settings],
            backstory: `A ${randomType.toLowerCase()} in a ${settings[setting as keyof typeof settings].toLowerCase()}`,
...
function generateAbilities(role: string): string[] {
    const abilityMap = {
    ...
    return abilityMap[role as keyof typeof abilityMap] || ["Basic Attack", "Dodge", "Item Use"];
}

function generateMotivation(): string {
    const motivations = [
...
function handleGenerateMechanic(category: string) {
    const mechanics = gameMechanics[category as keyof typeof gameMechanics] || [];
...
function handleBalanceWeapon(weaponData: any) {
    const suggestion = weaponBalancing.suggestBalance(weaponData.type, weaponData.stats);
    const dps = weaponBalancing.calculateDPS(
        weaponData.stats.damage, 
        weaponData.stats.speed, 
        weaponData.stats.critRate, 
        2.0
    );
    
    vscode.window.showInformationMessage(
        `DPS: ${dps}\nBalance Suggestions: ${suggestion}`, 
        'Insert Analysis'
    ).then(selection => {
...
function handleGenerateLevel(size: string, theme: string) {
...
function handleCreateCharacter(role: string, setting: string) {
